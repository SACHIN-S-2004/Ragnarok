...
export default function RagnarokLandingPage({ assetBase = "./assets" }) {
  ...
  const loreItems = [
    { q: "HOW WAS THE COSMOS CREATED FROM FIRE & ICE?", a: "In the primordial void Ginnungagap, the blazing heat of Muspelheim met the freezing mist of Niflheim, melting into the first giant Ymir and the cosmic cow Auðumbla." },
    { q: "WHY DID ODIN SACRIFICE HIS EYE & HANG UPON YGGDRASIL?", a: "Odin gave his eye to Mímir's well for a draught of supreme wisdom, and hung nine days from the World Tree pierced by his own spear to unlock the power of the runes." },
    { q: "WHAT IS THE FATE OF THOR & JÖRMUNGANDR AT RAGNARÖK?", a: "Thor will strike down the World Serpent Jörmungandr with Mjölnir, but after taking nine steps away from the venomous beast, he succumbs to its poison." }
  ];
  ...
  return <>
    ...
    <main className="rk-shell">
      ...
      <section className="rk-section rk-lore-archive">
        <small>KNOWLEDGE OF THE ANCIENTS</small>
        <h2>MYTHIC <em>CHRONICLES</em></h2>
        <div className="rk-lore-accordion">
          {loreItems.map((item, idx) => (
            <div key={idx} className={`rk-accordion-item ${activeLore === idx ? "rk-acc-open" : ""}`} onClick={() => setActiveLore(activeLore === idx ? null : idx)}>
              <div className="rk-acc-header">
                <span>ᛉ {item.q}</span>
                <span className="rk-acc-icon">{activeLore === idx ? "−" : "+"}</span>
              </div>
              {activeLore === idx && <p className="rk-acc-body">{item.a}</p>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="rk-final">
        <small>THE END IS WRITTEN. THE FUTURE IS YOURS.</small>
        <h2>RAGNARÖK</h2>
        <div className="rk-prophecy-grid">
          <div className="rk-lore-card">
            <span className="rk-card-rune">ᛟ</span>
            <h3>3D REALM EXPERIENCE</h3>
            <p>Step through the veil into an interactive Three.js atmospheric exploration of Asgard, Muspelheim, Hel, and all 9 Norse worlds.</p>
            <button className="rk-button" onClick={() => window.location.href = "/realms"}>
              ENTER 3D REALMS →
            </button>
          </div>
          <div className="rk-lore-card">
            <span className="rk-card-rune">ᚠ</span>
            <h3>THE PROPHECY OF VÖLUSPÁ</h3>
            <p>“An axe-age, a sword-age, shields are cloven. A wind-age, a wolf-age, ere the world falls. Yet from the ash, a green world rises anew.”</p>
            <span className="rk-card-author">— Ancient Poetic Edda</span>
          </div>
          <div className="rk-lore-card">
            <span className="rk-card-rune">ᛉ</span>
            <h3>THE VALHALLA CHRONICLES</h3>
            <p>Where 540 doors open to greet the fallen warriors, gathered by Odin’s Valkyries to stand together when the Gjallarhorn sounds.</p>
            <button className="rk-text-button" onClick={() => scrollTo("hero")}>
              READ SAGAS AGAIN ↑
            </button>
          </div>
        </div>
        <div className="rk-final-actions">
          <button className="rk-button" onClick={() => window.location.href = "/realms"}>EXPLORE 3D EXPERIENCE →</button>
          <button className="rk-button" onClick={() => scrollTo("hero")}>RETURN TO TOP ↑</button>
        </div>
      </section>

      <footer className="rk-stylish-footer">
        <div className="rk-footer-top">
          <div className="rk-footer-brand">
            <span className="rk-mark">ᛉ</span>
            <span className="rk-brand-title">RAGNARÖK</span>
          </div>
          <p className="rk-footer-tagline">An Interactive Mythological Archive & Cinematic Realm Experience</p>
          <div className="rk-footer-runes">ᚠ ᚢ ᚦ ᚨ ᚱ ᚲ ᚷ ᚹ ᚺ ᚾ ᛁ ᛃ ᛇ ᛈ ᛉ ᛊ ᛏ ᛒ ᛖ ᛗ ᛚ ᛜ ᛞ ᛟ</div>
        </div>

        <div className="rk-footer-grid">
          <div className="rk-footer-col">
            <h4>THE NINE REALMS</h4>
            <ul>
              <li><button onClick={() => window.location.href = "/realms"}>Asgard (Divine)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Vanaheim (Fertile)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Alfheim (Light)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Midgard (Mortal)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Jötunheim (Giants)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Svartalfheim (Forges)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Muspelheim (Fire)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Niflheim (Ice)</button></li>
              <li><button onClick={() => window.location.href = "/realms"}>Hel (Underworld)</button></li>
            </ul>
          </div>
          ...
          <div className="rk-footer-col">
            <h4>THE PANTHEON</h4>
            <ul>
              <li><button onClick={() => scrollTo("gods")}>Odin — The All-Father</button></li>
              <li><button onClick={() => scrollTo("gods")}>Thor — The Storm-Bearer</button></li>
              <li><button onClick={() => scrollTo("gods")}>Freyja — The Seeress</button></li>
              <li><button onClick={() => scrollTo("gods")}>Loki — The Shape-Changer</button></li>
              <li><button onClick={() => scrollTo("gods")}>Heimdall — The Watchman</button></li>
              <li><button onClick={() => scrollTo("gods")}>Týr — The Lawgiver</button></li>
            </ul>
          </div>
          ...
          <div className="rk-footer-col">
            <h4>ANCIENT SAGAS</h4>
            <ul>
              <li><button onClick={() => scrollTo("sagas")}>01. Odin’s Sacrifice</button></li>
              <li><button onClick={() => scrollTo("sagas")}>02. Thor & Jörmungandr</button></li>
              <li><button onClick={() => scrollTo("sagas")}>03. The Binding of Fenrir</button></li>
              <li><button onClick={() => scrollTo("sagas")}>04. Baldr’s Death</button></li>
              <li><button onClick={() => scrollTo("sagas")}>05. The Æsir–Vanir War</button></li>
              <li><button onClick={() => scrollTo("sagas")}>06. Ragnarök Twilight</button></li>
            </ul>
          </div>

          <div className="rk-footer-col rk-newsletter-col">
            <h4>THE SHIELDWALL DISPATCH</h4>
            <p>Pledge your oath to receive monthly lore drops, rune translations, and realm updates.</p>
            <form className="rk-footer-form" onSubmit={(e) => { e.preventDefault(); alert("Your oath is recorded in the Valhalla Codex!"); }}>
              <input type="email" placeholder="ENTER YOUR EMAIL..." required />
              <button type="submit" className="rk-button">PLEDGE</button>
            </form>
          </div>
        </div>

        <div className="rk-footer-bottom">
          <p>RAGNARÖK © 2026. ANCIENT STORIES REIMAGINED. ALL MYTHIC RIGHTS PRESERVED.</p>
          <div className="rk-footer-bottom-links">
            <button onClick={() => scrollTo("runes")}>Rune Transliteration</button>
            <span>•</span>
            <button onClick={() => window.location.href = "/realms"}>3D Realm Portal</button>
            <span>•</span>
            <button onClick={() => scrollTo("hero")}>Valhalla Archives</button>
          </div>
        </div>
      </footer>
    </main>
...
@media(hover:hover) and (pointer:fine){body,.rk-shell,.rk-shell button,.rk-shell a,.rk-shell input{cursor:none!important}}
.rk-cursor-ring{position:fixed;top:0;left:0;width:44px;height:44px;margin-top:-22px;margin-left:-22px;pointer-events:none;z-index:9999;display:flex;align-items:center;justify-content:center;will-change:transform;transition:width .3s cubic-bezier(.22,1,.36,1),height .3s cubic-bezier(.22,1,.36,1),margin .3s cubic-bezier(.22,1,.36,1),opacity .3s}
.rk-cursor-ring.rk-hidden,.rk-cursor-dot.rk-hidden{opacity:0}
.rk-cursor-svg{width:100%;height:100%;animation:rk-cursor-spin 16s linear infinite}
.rk-cursor-circle-bg{fill:rgba(138,99,62,.06);stroke:rgba(138,99,62,.3);stroke-width:1}
.rk-cursor-circle{fill:none;stroke:var(--rk-bronze);stroke-width:1.5;stroke-dasharray:8 4}
.rk-cursor-tick{stroke:var(--rk-ivory);stroke-width:1.5;opacity:.7}
.rk-cursor-rune{position:absolute;font-family:'Cormorant Garamond',serif;font-size:11px;color:var(--rk-bronze);pointer-events:none;opacity:.6}
.rk-cursor-ring.rk-hover{width:60px;height:60px;margin-top:-30px;margin-left:-30px}
.rk-cursor-ring.rk-hover .rk-cursor-circle-bg{fill:rgba(138,99,62,.16);stroke:var(--rk-bronze)}
.rk-cursor-ring.rk-hover .rk-cursor-circle{stroke:var(--rk-ivory);stroke-width:2}
.rk-cursor-ring.rk-click{transform:scale(.85)}
.rk-cursor-dot{position:fixed;top:0;left:0;width:10px;height:10px;margin-top:-5px;margin-left:-5px;pointer-events:none;z-index:10000;will-change:transform;display:flex;align-items:center;justify-content:center;transition:opacity .3s}
.rk-cursor-core{width:5px;height:5px;border-radius:50%;background:var(--rk-ivory);box-shadow:0 0 10px var(--rk-bronze),0 0 4px #fff;transition:transform .25s,background-color .25s}
.rk-cursor-dot.rk-hover .rk-cursor-core{transform:scale(1.8);background:var(--rk-bronze);box-shadow:0 0 14px var(--rk-ivory)}
.rk-cursor-dot.rk-click .rk-cursor-core{transform:scale(.6)}
@keyframes rk-cursor-spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}
  `;
